...
class Triangle:
    ...
    @nb.njit
    def draw_uv_nb(tampon_SSAA,texture,points_to_fill,p1,p2,p3,uv1,uv2,uv3,ambient_intensity,ambient_color,spot_color,N,L,coeff_diffus,D,k):
        ...
        for i in range(0,len(points_to_fill),2):
            ...
            color = texture[int(x_u*twidth),int(y_u*theight)]
            color_after_light = calcul_light(color,ambient_intensity,ambient_color,spot_color,N,L,coeff_diffus,D,k)
            tampon_SSAA[y,x] = color_after_light #On applique la texture donc ici appliqué la light
...
@nb.njit
def calcul_light(color,ambient_intensity,ambient_color,spot_color,N,L,coeff_diffus,D,k):
    return (ambient_light(color,ambient_intensity,ambient_color) + phong(color,spot_color,N,L,coeff_diffus,D,k))


@nb.njit
def ambient_light(color,ambient_intensity,ambient_color):
    return color*ambient_intensity*ambient_color

@nb.njit
def phong(color,spot_color,N,L,coeff_diffus,D,k):
    R = np.cross(-L,np.dot(N,L)*N)
    return diffus(color,spot_color,N,L,coeff_diffus) + speculaire(spot_color,R,D,k)

@nb.njit
def diffus(color,spot_color,N,L,coeff_diffus):
    """
    N : Vecteur Normal (Normalisé)
    L: Vecteur Lumière (Normalisé)
    spot_color : Couleur de la lumière entre 0 et 1
    coeff_diffus : A quels point l'objet est sensible au diffus (entre 0 et 1)
    """
    theta = np.dot(N,L)
    if theta < 0:
        return (color*0).astype(np.int16)
    else:
        return (color.astype(np.float32)*spot_color.astype(np.float32)*theta*coeff_diffus).astype(np.int16)
    


@nb.njit
def speculaire(spot_color,R,D,k):
    return (spot_color*(np.dot(R,D)**k)).astype(np.int16)